      ...
      <div className="flex items-center gap-2">
        ...
        {value && (
          <button type="button" onClick={() => { onChange(''); setOpen(false) }}
            className="ml-auto text-white/30 hover:text-white/60 transition-colors text-xs">Clear</button>
        )}
        {!value && (
...
export default function AddSubscriptionModal({ onClose, onSaved, sub = null }) {
  const isEdit = !!sub
  const { user } = useAuth()
  ...
  const [receiverId, setReceiverId] = useState(sub?.receiver_id    ?? '')
  const [amount,     setAmount]     = useState(sub?.amount         ?? '')
  const [billingDay, setBillingDay] = useState(sub?.billing_day    ?? 1)
  ...
  async function handleSave() {
    ...
    const { error } = isEdit
      ...
      : await supabase.from('subscriptions').insert({ ...payload, user_id: user.id, status: 'active' })
    if (error) { console.error('subscriptions save error:', error.message); setSaving(false); return }
    setSaving(false)
    onSaved()
    onClose()
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    await supabase.from('subscription_payments').delete().eq('subscription_id', sub.id)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
      ...
      <div className="glass-popup border border-white/10 rounded-2xl w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/8 shrink-0">
          <h2 className="text-base font-semibold text-white">{isEdit ? 'Edit Subscription' : 'Add Subscription'}</h2>
        ...
        <div className="overflow-y-auto flex flex-col gap-5 p-6 scrollbar-thin">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Netflix" className={inp} />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <ReceiverCombobox receiverId={receiverId} onReceiverSelect={setReceiverId}
              receivers={receivers} onAddReceiver={handleAddReceiver} inputClass={inp} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs text-muted uppercase tracking-widest">Billing day</label>
              <select value={billingDay} onChange={e => setBillingDay(e.target.value)} className={sel}>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="button" onClick={handleSave} disabled={saving || !name.trim() || !amount}
