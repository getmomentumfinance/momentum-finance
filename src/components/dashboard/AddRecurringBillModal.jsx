        ...
        <div className="absolute top-full left-0 right-0 mt-1 glass-popup border border-white/15 rounded-xl overflow-hidden z-20 shadow-xl">
          ...
          <div className="max-h-44 overflow-y-auto scrollbar-thin">
            {!q && <button type="button" onClick={() => { onChange(''); setOpen(false) }}
              className="w-full px-3 py-2.5 text-left text-sm text-white/25 hover:bg-white/5 transition-colors">None</button>}
            {filtered.length === 0
...
export default function AddRecurringBillModal({ onClose, onSaved, bill = null }) {
  const isEdit = !!bill
  const { user } = useAuth()
  ...
  const [nextDueDate, setNextDueDate] = useState(bill?.next_due_date ?? todayStr())
  const [categoryId, setCategoryId] = useState(bill?.category_id    ?? '')
  const [subId,      setSubId]      = useState(bill?.subcategory_id ?? '')
  ...
  async function handleSave() {
    ...
    if (!name.trim() || !normalizedAmount || isNaN(parseFloat(normalizedAmount))) return
    setSaving(true)
    const isNonMonthly = frequency === 'quarterly' || frequency === 'yearly'
    const payload = {
      name:           name.trim(),
      icon:           icon        || null,
      ...
      frequency,
      due_day:        isNonMonthly ? 1 : Number(dueDay),
      next_due_date:  isNonMonthly ? (nextDueDate || null) : null,
    ...
    const { error } = isEdit
      ...
      : await supabase.from('recurring_bills').insert({ ...payload, user_id: user.id })
    if (error) {
      console.error('recurring_bills save error:', error.message, error)
      setSaving(false)
      return
    }
    setSaving(false)
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    await supabase.from('recurring_bill_payments').delete().eq('bill_id', bill.id)
    ...
    >
      <div className="glass-popup border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-xl max-h-[92vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/8 shrink-0">
          <h2 className="text-base font-semibold text-white">{isEdit ? 'Edit Recurring Bill' : 'Add Recurring Bill'}</h2>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex flex-col gap-5 p-6 scrollbar-thin">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-white/30 pointer-events-none">{symbol}</span>
              <input
                value={amount}
          ...
          {/* Frequency + Due day / Next due date */}
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-2">
              <label className="text-xs text-muted uppercase tracking-widest">Frequency</label>
            ...
            <div className="flex flex-col gap-2">
              {frequency === 'monthly' ? (
                <>
                  <label className="text-xs text-muted uppercase tracking-widest">Due day</label>
                  <select value={dueDay} onChange={e => setDueDay(e.target.value)} className={sel}>
                    {DUE_DAYS.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                </>
              ) : (
                <>
                  <label className="text-xs text-muted uppercase tracking-widest">Next due date</label>
                  <input
                    type="date"
                    value={nextDueDate}
          ...
          {/* Reminder */}
          <div className="flex flex-col gap-2">
            <label className="text-xs text-muted uppercase tracking-widest">Reminder</label>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="button"
              onClick={handleSave}
              disabled={saving || !name.trim() || !amount || isNaN(parseFloat(String(amount).replace(',', '.')))}
