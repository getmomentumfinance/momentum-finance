...
export default function AddPlannedBillModal({ onClose, onSaved, item = null }) {
  const isEdit = !!item
  const { user } = useAuth()
  ...
  async function handleSave() {
    ...
    const { error } = isEdit
      ...
      : await supabase.from('planned_bills').insert({ ...payload, user_id: user.id, status: 'pending' })
    if (error) { console.error('planned_bills save error:', error.message); setSaving(false); return }
    setSaving(false)
    onSaved()
    onClose()
  }
  ...
  async function handleDelete() {
    ...
    await supabase.from('planned_bills').delete().eq('id', item.id)
    setDeleting(false)
    onSaved()
    onClose()
  }
    ...
    >
      <div className="glass-popup border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-xl max-h-[92vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/8 shrink-0">
          <h2 className="text-base font-semibold text-white">{isEdit ? 'Edit Planned Bill' : 'Add Planned Bill'}</h2>
        ...
        <div className="overflow-y-auto flex flex-col gap-5 p-6 scrollbar-thin">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Car insurance" className={inp} />
          </div>
